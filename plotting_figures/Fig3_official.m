...
for idx_x = 1:length(x_loc2plt)
    ...
    for i_c = 1:length(int_ctrl)
        plot(ax_spkt, int_ctrl(i_c), -1, 'ok', 'MarkerSize',4)
    end
    ...
    plot(ax_tgt, tscale_start*[1,1], pspscale_start+[0,psp_scale], ...
        '-k', 'LineWidth', 0.75);
    title(ax_tgt, ['\Deltat_{inp} = ' num2str(x_vec(i_x)) 'ms'], 'fontweight', 'normal', 'fontsize', 10, 'visible', 'on')
